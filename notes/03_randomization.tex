...
\begin{proof}
Let $L \in \BPP$. By success amplification, there is a TM $M$ such that $L(M) = L$ with error smaller than $\frac{1}{2^n}$. Choose an $x \in \{0, 1\}^n$. Therefore, $\Pr[\text{$M$ computes incorrectly on $x$}] < \frac{1}{2^n}$. We can observe that:
\[
